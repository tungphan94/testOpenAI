import { SearchState } from "../llm/services/sessionManager";
import { isStringArray, isStringOrNull } from "./buildStateKnow";
...
): SearchState
{
  ...
  for (const p of patch)
  {
     if(p.op === "set"){
        switch (p.path) {
          case "/criteria/city":
              if(isStringOrNull(p.value)) next.city = p.value;
              break;
          case "/criteria/place":
              if(isStringOrNull(p.value)) next.place = p.value;
              break;
          case "/criteria/facility_type":
              if(isStringOrNull(p.value)) next.facility_type = p.value;
              break;
          case "/criteria/specialty":
              if(isStringOrNull(p.value)) next.specialty = p.value;
              break;   
          case "/criteria/radius_meters":
              if(isStringOrNull(p.value)) next.radius_meters = p.value;
              break;   
          case "/criteria/include":
              if(isStringArray(p.value)) next.include = p.value;
                break;
        }
     }
  }
